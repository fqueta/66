@if (empty($bidding->id))
	{!!Form::open([
		'files' => true,
		'route' => ['admin.b_trimestrals.store'],
		'method' => 'POST',
		'class' => 'ls-form row'
	])!!}
@else
	{!!Form::open([
		'files' => true,
		'route' => ['admin.b_trimestrals.update', $bidding->id],
		'method' => 'PUT',
		'class' => 'ls-form row'
	])!!}
	{!!Form::model($bidding, ['route' => ['admin.b_trimestrals.update', $bidding->id]]) !!}
...
@endif
	<div class="content-editor">
		...
		<fieldset>
			<label class="ls-label col-md-7">
				...
				{!!Form::textarea('object', old('object'), ['placeholder' => 'Descrição'])!!}
			</label>
		</fieldset>
		@if($status !== 'creating')
			<!--
	...
	<hr>
	@include('elements.admin.shared.form_actions', ['route_cancel' => route('admin.b_trimestrals.index')])
{!!Form::close()!!}
@section('scripts')
